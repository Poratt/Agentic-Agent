...
describe('IdeasHistory', () => {
    ...
    beforeEach(async () => {
        ...
        await TestBed.configureTestingModule({
            ...
            providers: [provideZonelessChangeDetection(), { provide: IdeasStore, useValue: storeMock }],
        }).compileComponents();
    ...
    it('triggerNightly should render the success message', async () => {
        await component.triggerNightly();
        fixture.detectChanges();
        expect(component.triggerSuccess()).toBe('Done');
        expect(fixture.nativeElement.querySelector('.banner-content')?.textContent).toContain('Done');
    });

    it('triggerNightly banner should animate out then clear itself after ~5s', async () => {
        vi.useFakeTimers();
        try {
            await component.triggerNightly();
            fixture.detectChanges();
            expect(component.bannerClosing()).toBe(false);

            vi.advanceTimersByTime(5000);
            expect(component.bannerClosing()).toBe(true);

            vi.advanceTimersByTime(300);
            expect(component.triggerSuccess()).toBeNull();
        } finally {
            vi.useRealTimers();
        }
    });
