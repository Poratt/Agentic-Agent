...
- Current active todo: `documents/features/todo/database-storage-monitor-plan.md` (monitor DB storage) and **Phase 9 – Settings UI (LLM admin) – in progress**.
...
- Fixed: Phase 8b build errors in `frontend/src/app/features/settings/settings.ts` and `frontend/src/app/core/store/llm-admin.store.ts`. `modelForm.contextWindow` now uses `null as number | null` so `openModelEdit()` can pass `model.contextWindow ?? null`. Added missing `createModel(providerId, dto)` and `updateModel(id, dto)` actions to `LlmAdminStore`, mirroring the existing provider actions. Cleaned a duplicate dead `loadProviders()` block inside `setDefaultModel(...)` that was double-firing HTTP requests. Verification: `npx ng build` from `frontend` passes; settings chunk is 11.39 kB. Remaining warnings are unchanged pre-existing ones (`explorer.css`, `chat-message.css`).
- Added: empty-state CTA button "צור ספק חדש" that calls the existing `openCreate()` method. Verification: `npx ng build` passes; settings chunk 11.60 kB.
- Fixed: empty-state CTA was a no-op because no `<p-dialog>` was rendered in `settings.html`. Added the missing provider create/edit PrimeNG dialog (header switches by `isEdit()`, fields: key ltr readonly-on-edit / label / baseUrl ltr / apiKey ltr password with keep-existing placeholder on edit / active checkbox) and the missing model create/edit PrimeNG dialog (name ltr readonly-on-edit / label / contextWindow number / sortOrder number / three checkboxes). Both dialogs sit outside the `@switch` block as siblings of the empty/error/loading/ready cases so they are reachable from any page state. Added two get/set accessors (`dialogVisibleModel`, `modelDialogVisibleModel`) on `Settings` so `[(visible)]` can drive the existing visibility signals and call the existing cancel handlers when the dialog closes via Esc / backdrop / X. Reused global `.form`, `.form-group`, `.form-group-checkbox`, `.checkbox-label`, `.primary-btn.filled`, `.transparent-btn`, and `dir="ltr"` on technical identifiers per the LLM Provider DB plan. Verification: `npx ng build` from `frontend` passes in 6.1s; settings chunk is 17.09 kB. Remaining warnings are unchanged pre-existing ones (`explorer.css`, `chat-message.css`).
- Root cause note: the page was rendering the empty state because the DB genuinely has no providers. `LlmProviderSeedService` skipped silently because `backend/.env` has no `LLM_PROVIDER_*_LABEL` / `LLM_PROVIDER_*_BASE_URL` vars. The user can either (a) set those env vars per `backend/.env.example` and restart the backend, or (b) use the new provider create dialog to add a row manually. Either way, the Settings page UI is now end-to-end functional.
- Next exact step: visually click the "צור ספק חדש" button on the empty state, confirm the provider dialog opens, save a provider, and confirm the page jumps to Ready with the new row. Then click `+` on a row to test the model create dialog. Then Phase 8b: render the per-provider expanded row using `llmAdminStore.models(providerId)` / `modelsLoading(providerId)` / `modelsError(providerId)`, and wire the default-model assignment dropdown. Then Phase 9 polish (Hebrew labels review, RTL layout, no component CSS).
