...
- **סטטוס**: ✅ **טופל** (2026-08-08) — roleGuard חדש (`CanActivateFn`) עם `data: { roles: [UserRole.Admin] }` על `/dashboard` ו-`/users`. משתמש רגיל שמגיע לכתובת מופנה ל-`/chat`. בנוסף: כפתורי "לוח בקרה" ו"ניהול משתמשים" מוסתרים מה-sidebar למשתמשים רגילים (isAdmin computed).
...
- **סטטוס**: ✅ **טופל** (2026-08-08) — סיסמה מ-`ADMIN_PASSWORD` env; ב-production без env — סיסמה אקראית + אזהרה להחלפה; ב-dev fallback `changeme`; לא מדפיס credentials ללוג פרט למקרה של סיסמה אקראית-generated (פעם אחת בלבד).
...
| L34               |       | 2h                       | עמודת סידור יציב (sequence/order) ל-chat_messages — במקום הסתמכות על timestamp                                                                                               | chat-message.entity.ts (מיגרציה)                                                               |
| L35               | 🤌    | 3h                       | soft-delete (deletedAt) ל-chat_sessions + chat_messages — עקביות עם LLM providers; עדכון כל ה-queries ל-WHERE deletedAt IS NULL                                              | chat-session.entity.ts, chat-message.entity.ts (מיגרציה)                                       |
| L36               |       | 2h                       | (אופציונלי) עמודת status (complete/streaming/failed) על chat_messages — לזיהוי הודעות תקועות באמצע סטרימינג                                                                  | chat-message.entity.ts (מיגרציה)                                                               |
| **סה"כ (L1-L36)** | 30/36 | **~12h ≈ 1.5 ימי פיתוח** |                                                                                                                                                                              |                                                                                                |
