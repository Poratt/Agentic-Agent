...
> Gray-release upgrade of Agnes 2.0 Flash with improved coding capability, agent workflows, tool calling, and multimodal understanding.

<Info>
  Agnes 2.5 Flash is a gray-release language model upgraded from Agnes 2.0 Flash. It is currently available only to selected gray-release users, and is not enabled for all accounts by default. It keeps the same OpenAI-compatible Chat Completions integration path while improving the experience for coding, agent workflows, tool calling, multi-turn conversations, reasoning, and image understanding.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Release Status" icon="flask">
    Gray release. Only selected gray-release users can access it.
  </Card>
...
<Warning>
  Agnes 2.5 Flash is currently released only to selected gray-release users. If your API key has not been enabled for this gray release, the model may not appear in the model list or may return an unavailable-model error. In that case, continue using `agnes-2.0-flash` as the stable fallback.
</Warning>
...
<Tip>
  For gray-release rollout, keep a fallback model in your application config. If `agnes-2.5-flash` is not enabled for the current user or API key, retry the same request with `agnes-2.0-flash`.
</Tip>
...
Agnes 2.5 Flash is in gray release and is only available to selected gray-release users. Availability, rate limits, and billing behavior follow the entitlement shown for your Agnes AI account and API key. If the model is not enabled for your key, use `agnes-2.0-flash` as the stable fallback.
...
<Check>
  Confirm that the current user or API key is included in the Agnes 2.5 Flash gray-release group.
</Check>
