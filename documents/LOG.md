...
## 2026-06-14 LLM Provider DB — Phase 8b build repair

- Repaired the Phase 8b Settings wiring that was blocking the frontend build. Three TypeScript errors and one latent HTTP double-fire were fixed.
- `frontend/src/app/features/settings/settings.ts`: the `modelForm` was initialized with `contextWindow: [null]`, which Angular's strict `FormGroup` typing infers as a control that only accepts `null`. `openModelEdit()` then tried to `setValue({ contextWindow: model.contextWindow ?? null })` where `model.contextWindow` is `number | null`, producing `TS2322: Type 'number | null' is not assignable to type 'null'`. Fixed by widening the form control initializer to `null as number | null`. The form still defaults to `null` for create flows and now accepts the real `number` on edit flows.
- `frontend/src/app/core/store/llm-admin.store.ts`: was missing the `createModel` and `updateModel` actions. The component code already called `this.llmAdminStore.createModel(this.modelProviderId, dto)` and `this.llmAdminStore.updateModel(this.editModelId, dto)`, producing `TS2339: Property 'createModel' does not exist on type 'LlmAdminStore'` and the equivalent for `updateModel`. Added both actions using the same pattern as `createProvider` / `updateProvider`: set `_loading` / `_error`, call the matching `LlmService` method, refresh state on success via `finalize` teardown. `createModel` reloads that provider's models (`loadModels(providerId)`), and `updateModel` reloads the providers list so the `modelsCount` and `defaultModelId` columns stay in sync.
- Same store: `setDefaultModel(providerId, modelId)` contained a duplicated dead block that called `getAdminProviders()` a second time after the primary `setDefaultModel` call. Removed the duplicate so changing a default model now fires exactly one PATCH plus the post-success reload instead of two. This was a pre-existing bug inside the function I had to touch, so I cleaned it; not a stylistic change.
- Verification: `npx ng build` from `frontend` passes in 5.9s. Settings chunk is 11.39 kB. Remaining warnings are the unchanged pre-existing `explorer.css` and `chat-message.css` budget overages already documented in `STATUS.md`.
- No new dependencies, no new component CSS, no new wrappers; the existing `page-content` / `page-header` / `glass-effect card` / `page-state` / `table-container` / `primary-btn` / `badge` shell is unchanged. No architecture diagram update was needed because the LLM admin API contract, the Settings module boundary, and the `LlmAdminStore` shape are all unchanged — only the form's nullability widened and the store gained the two missing actions.
- Files touched this session: `frontend/src/app/features/settings/settings.ts`, `frontend/src/app/core/store/llm-admin.store.ts`, `documents/HANDOFF.md`, `documents/STATUS.md`, and `documents/LOG.md`.

## 2026-06-14 LLM Provider DB — Phase 8b Settings dialog templates

- The Phase 8b empty-state CTA "צור ספק חדש" was a no-op: `openCreate()` flipped the `dialogVisible` signal to `true`, but no `<p-dialog>` was rendered anywhere in `settings.html`. Same gap existed for the model create/edit handlers — the form groups and the `.ts` methods were wired, but the PrimeNG dialog templates that listen to the signals were missing entirely.
- Root cause of the empty DB: hit `http://127.0.0.1:3000/llm/admin/providers` directly with the dev session — backend returns a valid empty list, not a 401. `LlmProviderSeedService` skipped silently because `backend/.env` has no `LLM_PROVIDER_*_LABEL` / `LLM_PROVIDER_*_BASE_URL` env vars; the seed only logs at `debug` level on skip, so it looked like the page was broken when it was actually rendering the right empty state.
- Added the two missing PrimeNG dialog templates to `frontend/src/app/features/settings/settings.html`, placed as siblings of the `<div dir="ltr">` wrapper that contains the `@switch`:
  - **Provider create/edit dialog** — header `ספק חדש` / `עריכת ספק` based on `isEdit()`. Fields: `key` (ltr, `[readonly]="isEdit()"` so the unique key can't be changed post-create), `label`, `baseUrl` (ltr, `type="url"`), `apiKey` (ltr, `type="password"`, placeholder `השאר ריק כדי לשמור את הקיים` on edit so the encrypted existing key isn't accidentally overwritten with empty), `active` (checkbox). Footer: `ביטול` (transparent) + `שמור` (primary-btn filled). All `formControlName`s match the existing `providerForm` group so `saveProvider()` / `cancelDialog()` work without any `.ts` changes.
  - **Model create/edit dialog** — header `מודל חדש` / `עריכת מודל` based on `isModelEdit()`. Fields: `name` (ltr, read-only on edit), `label`, `contextWindow` (number, nullable via the `null as number | null` initializer), `sortOrder` (number), `active` / `supportsStreaming` / `supportsTools` (three checkboxes). Same footer pattern. All `formControlName`s match `modelForm` so `saveModel()` / `cancelModelDialog()` work without `.ts` changes.
- Added two get/set accessors on `Settings` so `p-dialog`'s `[(visible)]` (which expects a writable property, not a signal) can drive the existing visibility signals and call the existing cancel handlers when the dialog closes via Esc / backdrop / X:
  - `dialogVisibleModel` get/set ↔ `dialogVisible` signal, calls `cancelDialog()` on close
  - `modelDialogVisibleModel` get/set ↔ `modelDialogVisible` signal, calls `cancelModelDialog()` on close
- No new component CSS, no new wrapper classes, no new dependencies. Reused global `.form`, `.form-group`, `.form-group-checkbox`, `.checkbox-label`, `.primary-btn.filled`, `.transparent-btn`, `.ph` icon font, and the existing `glass-effect` card shell. `dir="ltr"` is applied to technical identifiers (`key`, `baseUrl`, `apiKey`, model `name`) per the LLM Provider DB plan convention; all labels and other text stay in Hebrew/RTL.
- Verification: `npx ng build` from `frontend` passes in 6.1s. Settings chunk grew from 11.60 kB → 17.09 kB (the two dialog templates and their reactive form bindings). No new TS errors, no new CSS warnings, no new template warnings. The two pre-existing `explorer.css` and `chat-message.css` budget warnings remain unchanged.
- No architecture diagram update was needed because the dialogs are local Settings page UI plumbing; the LLM admin API contract, `LlmAdminStore` shape, and the Settings module boundary are all unchanged.
- Files touched this session: `frontend/src/app/features/settings/settings.ts`, `frontend/src/app/features/settings/settings.html`, `documents/HANDOFF.md`, `documents/STATUS.md`, and `documents/LOG.md`.
